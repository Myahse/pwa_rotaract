...
func (s *Server) buildRouter() (chi.Router, *scheduler.BirthdayScheduler, error) {
	...
	clubDueService := service.NewClubDueService(clubDueRepo, clubRepo, userRepo, fileStore, s.cfg.APIPublicURL)
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(protected chi.Router) {
			...
			protected.Route("/clubs/{clubID}", func(club chi.Router) {
				...
				club.Get("/me/access", clubHandler.GetMyClubAccess)
				...
				club.With(middleware.RequireClubPermission(clubRepo, "chat.groups.read")).Get("/chat/inbox", chatHandler.ListInbox)
				club.With(middleware.RequireClubPermission(clubRepo, "chat.groups.read")).Post("/chat/direct", chatHandler.CreateDirectChat)
				...
				club.With(middleware.RequireClubPermission(clubRepo, "club.members.assign_role")).Get("/role-assignments", clubHandler.ListRoleAssignments)
				...
				club.Post("/chat/groups", chatHandler.CreateClubGroup)
				club.Get("/dues", clubDueHandler.List)
				club.Post("/dues", clubDueHandler.Submit)
				club.Post("/dues/{dueID}/received", clubDueHandler.MarkReceived)
			...
			protected.Post("/chat/groups/{groupID}/members", chatHandler.AddGroupMembers)
