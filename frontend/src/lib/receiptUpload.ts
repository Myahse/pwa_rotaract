...
async function compressImage(file: File): Promise<PreparedReceipt> {
  ...
  return { blob, mimeType: 'image/jpeg', filename: `${base}.jpg`, size: blob.size }
}
...
export async function prepareReceiptFile(file: File): Promise<PreparedReceipt> {
  if (file.type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf')) {
    ...
    return { blob: file, mimeType: 'application/pdf', filename: file.name || 'receipt.pdf', size: file.size }
  }
  if (!file.type.startsWith('image/')) throw new Error('invalid_type')
  if (file.size > MAX_IMAGE_BYTES && file.type !== 'image/jpeg') return compressImage(file)
  ...
  const mimeType =
    file.type === 'image/png' || file.type === 'image/webp' || file.type === 'image/jpeg'
      ...
      : 'image/jpeg'
  const base = file.name.replace(/\.[^.]+$/, '') || 'receipt'
  return { blob: file, mimeType, filename: `${base}.${extensionForMime(mimeType)}`, size: file.size }
}
