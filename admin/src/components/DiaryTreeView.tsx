import type { ClubDiaryEntryType, ClubDiaryTreeNode, ClubMandateRole } from '../api/types'
...
const ROLE_LABELS: Record<ClubMandateRole, string> = {
  president: 'Président',
        ...
        <li key={node.id} className="diary-node">
          <div className={`diary-card type-${node.entry_type} ${node.last_name === 'Mandat' ? 'type-mandate' : ''}`}>
            ...
            <div>
              ...
              {node.organization && <p className="muted">{node.organization}</p>}
...
export { TYPE_LABELS, ROLE_LABELS }
